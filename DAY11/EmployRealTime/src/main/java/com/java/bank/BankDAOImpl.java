...
@Override
public String depositAccountDao(int accountNo, int depositeAmount)  throws ClassNotFoundException, SQLException {
	Bank bank = searchAccountDao(accountNo);
	if (bank!=null) {
		connection = BankConnectionHelper.getConnection();
		String cmd = "Update Bank set Amount = Amoubt + ? Where AccountNo=?";
	    pst = connection.prepareStatement(cmd);
